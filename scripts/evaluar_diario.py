...
# --------------------------------------------------------------- la ventana movil
def seleccionar_cohorte(datos, modo="continua", cobertura_min=.9):
    """Seleccion por disponibilidad, nunca por error ni ingresos.

    Continua: al menos el 90 % de los instantes del modelo con mas cobertura y
    presencia en el ultimo dia disponible. La comparacion posterior sigue usando
    la interseccion EXACTA, no presupone que igual conteo implica mismas fechas.
    """
    if modo not in ("continua", "todos") or not 0 < cobertura_min <= 1:
        raise ValueError("Politica de cohorte invalida")
    data = datos.assign(ts=indice_local(datos.ts))
    data = data[data.ts.notna() & np.isfinite(data[["pred", "real"]].to_numpy()).all(axis=1)]
    if data.duplicated(["model", "seed", "ts"]).any():
        raise ValueError("Instantes duplicados por modelo/semilla")
    if data.empty:
        return data, {"modo": modo, "cobertura_min": cobertura_min, "incluidos": [], "excluidos": []}
    groups = list(data.groupby(["model", "seed"]))
    maximum = max(len(g) for _, g in groups)
    latest = data.ts.max().date()
    included, excluded = [], []
    for (model, seed), g in groups:
        reasons = []
        if modo == "continua":
            if len(g) < maximum*cobertura_min:
                reasons.append("cobertura_insuficiente")
            if g.ts.max().date() != latest:
                reasons.append("sin_datos_ultimo_dia")
        identity = {"model": model, "seed": int(seed), "periodos_disponibles": len(g)}
        if reasons:
            excluded.append({**identity, "motivos": reasons})
        else:
            included.append(identity)
    keys = {(x["model"], x["seed"]) for x in included}
    chosen = data[[ (m, int(seed)) in keys for m, seed in zip(data.model, data.seed) ]]
    return chosen, {"modo": modo, "cobertura_min": cobertura_min,
                    "incluidos": included, "excluidos": excluded}


def ventana(con, datos: pd.DataFrame, real: pd.Series, escribir: bool, cohorte="continua"):
    ...
    datos, seleccion = seleccionar_cohorte(datos, cohorte)
    for item in seleccion["excluidos"]:
        _log("cohorte", f"{item['model']} / {item['seed']}: fuera del ranking ({', '.join(item['motivos'])})")
    ...
    if datos.empty:
        return 0
    spec = {**metadata_evaluacion(common), "cohorte": seleccion}
    filas = []
    ...
    if escribir and filas:
        with con.cursor() as cur:
            # prod_30d/global es una instantanea movil. Invalidar filas ausentes evita
            # que un modelo retirado conserve un ganador de una ventana antigua.
            # Los periodos de backtest y los resultados BESS historicos no se tocan.
            cur.execute("""
                UPDATE model_metrics SET n_obs=0, mae=NULL, rmse=NULL, smape=NULL,
                    pinball80=NULL, cobertura_ic80=NULL, captura_pct=NULL, eur_dia=NULL,
                    pico_1h_pct=NULL, skill_vs_naive=NULL, simulador=%s::jsonb,
                    calculado_en=now()
                WHERE periodo='prod_30d' AND corte='global'
            """, (json.dumps(spec),))
            cur.executemany("""
...
def main():
    ...
    ap.add_argument("--cohorte", choices=["continua", "todos"], default="continua",
                    help="continua: cobertura >=90%% del maximo y ultimo dia; todos: interseccion de todos")
    ...
    try:
        ...
        if datos.empty:
            ...
            return
        dias = sorted(set(indice_local(datos.ts).date))
        ...
        ventana(con, datos, real, escribir, a.cohorte)
    finally:
        con.close()
