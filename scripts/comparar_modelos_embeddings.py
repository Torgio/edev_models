"""
Compara "sin embedding" (matriz_nucleo original, preparar_tensores.preparar()
sin modificar) vs "con embedding" (matriz_nucleo_tensores, vía
preparar_con_embeddings()), usando las 8 familias x 3 semillas de
entrenar_finales.py.
...
def main(salida=SALIDA):
    ...
    print("\n=== Experimento CON embedding (matriz_nucleo_tensores) ===")
    T_con = preparar_con_embeddings(matriz="nucleo_tensores")
    df_con = correr_experimento("con_embedding", T_con, EF, keras, layers, salida)
 
    combinado = pd.concat([df_sin, df_con], ignore_index=True)
