...
O directamente desde la terminal, para generar los CSV compartidos:
    python construir_dataset_maestro.py                    # dataset HORARIO (por defecto)
...
def _features_forecast(conn) -> pd.DataFrame:
    ...
    cols = _filtrar_existentes(conn, "esios_forecast_da", COLS_SEGURAS_FORECAST, "forecast")
...
def _features_pdbc(conn) -> pd.DataFrame:
    ...
    diario = df.groupby("fecha")[cols].agg(["mean", "min", "max"])
    diario.columns = [f"pdbc_{c}_{stat}" for c, stat in diario.columns]
    lag1 = _construir_lag(diario, 1, "lag1d")
    lag7 = _construir_lag(diario, 7, "lag7d")
...
def _calendario(index_fechas) -> pd.DataFrame:
    ...
    try:
        import holidays
        fest = holidays.Spain(years=range(d1.year.min(), d1.year.max() + 1))
        es_fest = np.array([d.date() in fest for d in d1])
    except ImportError:
        print("AVISO: falta el paquete `holidays` (pip install holidays). "
              "d1_is_festivo y d1_es_puente salen a 0 -- se pierde una feature relevante.")
        es_fest = np.zeros(len(d1), dtype=bool)
...
COLS_HORARIAS_FORECAST = COLS_SEGURAS_FORECAST
...
def construir_dataset_horario(incluir_clima: bool = False,
    ...
    try:
        ...
        # ── precio a la misma hora de D, D-1 y D-6 (mismo dia de la semana que D+1) ────────
        pr_D = _desplazar_dias(precio, 1, "D")
        pr_Dm1 = _desplazar_dias(precio, 2, "Dm1")
        pr_Dm6 = _desplazar_dias(precio, 7, "Dm6")
        ...
        # el dia D NO entra: a las 12:00 de D solo han ocurrido las horas 00-11
        re_Dm1 = _desplazar_dias(real, 2, "Dm1")
        re_Dm6 = _desplazar_dias(real, 7, "Dm6")
...
def construir_tensores(ventana_dias: int = 7, incluir_clima: bool = False,
    ...
    ok = (~np.isnan(y).any(axis=1)) & ok_bloque
...
if __name__ == "__main__":
    ...
    if hacer_horario:
        print("Construyendo dataset horario (formato largo, para Seq2Seq y arboles)...")
        dfh = construir_dataset_horario(incluir_clima=args.con_clima)
        ...
        ruta_h = _ruta_versionada(carpeta, "dataset_horario", **comun)
        _escribir_con_meta(dfh, ruta_h, "1 fila por (dia objetivo D+1, hora)",
                           incluir_clima=args.con_clima,
                           filas={"train": len(trh), "validation": len(vah), "test": len(teh)},
        ...
        print(f"guardado en: {ruta_h.name}  (+ {ruta_h.with_suffix('.meta.json').name})")
