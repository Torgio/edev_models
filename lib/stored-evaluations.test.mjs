...
test('headline candidates require the maximum coverage of their group', () => {
  const rows = [
    row({ model: 'complete-a', n_obs: 720, captura_pct: 91 }),
    row({ model: 'complete-b', n_obs: 720, captura_pct: 90 }),
    row({ model: 'short', n_obs: 72, captura_pct: 99 }),
  ];
  assert.deepEqual(maxCoverageEvaluations(rows).map(x => x.model), ['complete-a', 'complete-b']);
  assert.deepEqual(maxCoverageEvaluations([row({ n_obs: null })]), []);
});
