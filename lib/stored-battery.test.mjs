...
import { batteryModels, batteryRecordState, preferredBatteryModel, storedPlanSummary } from './stored-battery.ts';
...
test('battery availability distinguishes every stored-data state', () => {
  assert.equal(batteryRecordState(0, 0), 'empty');
  assert.equal(batteryRecordState(24, 0), 'plan-only');
  assert.equal(batteryRecordState(0, 1), 'result-only');
  assert.equal(batteryRecordState(24, 1), 'complete');
});
