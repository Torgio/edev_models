...
test('missing performance is unavailable rather than favourable', () => {
  assert.equal(performanceTone(null), 'unavailable');
  assert.equal(performanceTone(undefined), 'unavailable');
  assert.equal(performanceTone(Number.NaN), 'unavailable');
  assert.equal(performanceTone(0), 'good');
  assert.equal(performanceTone(-0.1), 'bad');
});
