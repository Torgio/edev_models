...
export async function proxyBatteryStudy(request: Request, path: string, options: {
  ...
  if (needsProductionDiscovery && match && !availableIds.includes(Number(match[2]))) {
    try {
      const catalogue = await (options.fetcher ?? fetch)(new URL('ejecuciones', studyBase), {
        method: 'GET', headers, redirect: 'manual', cache: 'no-store', signal: AbortSignal.timeout(20000),
      });
      if (!catalogue.ok || !catalogue.headers.get('content-type')?.includes('application/json')) return reply('No se pudo consultar el catálogo de estudios.', 502);
      const catalogueData = await catalogue.json() as { runs?: Array<{ run_id?: unknown; code?: unknown }> };
      const discovered = (catalogueData.runs ?? []).filter(item => Number.isSafeInteger(item.run_id)
        && typeof item.code === 'string' && item.code.startsWith('WEB-')).map(item => item.run_id as number);
      if (!discovered.includes(Number(match[2]))) return reply('No hay datos guardados para este estudio o tramo.', 404);
    } catch { return reply('No se pudo consultar el catálogo de estudios.', 502); }
  }
