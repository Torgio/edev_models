...
function pricePerMwh(value: number) {
  return `${priceFormatter.format(value)} €/MWh`;
}
...
export function marketAlerts(current: Array<number | null>, comparison: Array<number | null> = []) {
  ...
  if (peak && peak.value >= 150) alerts.push({ id: 'peak', title: 'Pico relevante', detail: `Máximo de ${pricePerMwh(peak.value)}`, from: peak.index, to: peak.index, value: peak.value });
  ...
  if (ramp) alerts.push({ id: 'ramp', title: 'Rampa rápida', detail: `Subida de ${pricePerMwh(ramp.value)}`, ...ramp });
  ...
  if (gap && Math.abs(gap.value) >= 35) alerts.push({ id: 'difference', title: 'Diferencia entre días', detail: `${gap.value >= 0 ? '+' : ''}${pricePerMwh(gap.value)} frente al día comparado`, from: gap.index, to: gap.index, value: gap.value });
  return alerts;
}
