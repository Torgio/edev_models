...
export function BatteryStudyLaunch({ studyName, battery, period, onPeriodChange, onBusyChange, availableFrom, availableTo, consumptionCode, generationCode, generationIncluded, onBack: back, onComplete }: { studyName: string; battery: BatteryDraft; period: StudyPeriod; onPeriodChange: (period: StudyPeriod) => void; onBusyChange: (busy: boolean) => void; availableFrom: string; availableTo: string; consumptionCode: string; generationCode: string; generationIncluded: boolean; onBack: () => void; onComplete: (runId: number) => void }) {
  ...
  const policyExplanation = policy === 'solo_excedente'
    ? 'La batería solo puede cargarse con excedente solar que no cubre el consumo de esa hora. No compra energía de la red; sin excedente, no carga.'
    : policy === 'prefiere_excedente'
      ? 'Prioriza cargar con excedente solar. Puede comprar de la red si el ahorro esperado lo justifica, pero con una penalización para que no sea la primera opción.'
      : 'El optimizador puede cargar desde excedente solar o desde la red cuando resulte más conveniente según los precios, el consumo y los límites de la instalación.';
