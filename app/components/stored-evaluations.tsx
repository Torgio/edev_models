'use client';
import '@/app/evaluation-report.css';
...
export function StoredEvaluations({ onSessionExpired }: { onSessionExpired: () => void }) {
  ...
  return <><section className="evaluation-section" id="hitos" aria-labelledby="evaluation-title">
    ...
    {preview && <div className="report-preview-controls"><Button onClick={async () => { await document.fonts.ready; window.print(); }}>Imprimir / Guardar PDF</Button><Button variant="outline" onClick={() => setPreview(false)}>Cerrar vista previa</Button></div>}
    <section className={`evaluation-print-report ${preview ? 'is-preview' : ''}`} aria-label="Vista previa del informe de evaluación"><EvaluationReport rows={ranked} /></section><PerformanceHistory onSessionExpired={onSessionExpired} /></>;
}
